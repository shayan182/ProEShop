...
public class ConsignmentService : GenericService<Consignment>, IConsignmentService
{
    ...
    public ConsignmentService(IUnitOfWork uow, IMapper mapper)
        : base(uow)
    {
        _mapper = mapper;
    ...
    {
        var consignments = _consignments.AsQueryable().AsNoTracking();
        ...
        return new()
        {
            Consignments = await _mapper.ProjectTo<ShowConsignmentViewModel>(paginationResult.Query)
                .ToListAsync(),
